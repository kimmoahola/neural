...
# https://github.com/mattm/simple-neural-network/pull/6
    ...
    def train(self, training_inputs, training_outputs):
        ...
        for i in range(len(self.hidden_layer)):
            pd_errors_wrt_hidden_neuron_total_net_input = [[0] * len(
                self.hidden_layer[-1 - i].neurons)] + pd_errors_wrt_hidden_neuron_total_net_input;
            for h in range(len(self.hidden_layer[-1 - i].neurons)):
                ...
                if i == 0:
                    ...
                    for o in range(len(self.hidden_layer[-i].neurons)):
                        d_error_wrt_hidden_neuron_output += pd_errors_wrt_hidden_neuron_total_net_input[1][o] * \
        ...
        for l in range(len(self.hidden_layer)):
            for h in range(len(self.hidden_layer[i].neurons)):
                ...
                for w_ih in range(len(self.hidden_layer[i].neurons[h].weights)):
                    # ∂Eⱼ/∂wᵢ = ∂E/∂zⱼ * ∂zⱼ/∂wᵢ
                    pd_error_wrt_weight = pd_errors_wrt_hidden_neuron_total_net_input[i][h] * \
                                          self.hidden_layer[i].neurons[h].calculate_pd_total_net_input_wrt_weight(w_ih)
...
nn = NeuralNetwork(2, [2, 2], 2, hidden_layer_weights=[[0.15, 0.2, 0.25, 0.3], [0.15, 0.2, 0.25, 0.3]],
                   hidden_layer_bias=[0.35, 0.35], output_layer_weights=[0.4, 0.45, 0.5, 0.55], output_layer_bias=0.6)
