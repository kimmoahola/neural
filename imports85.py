...
def dataset(y_name="should-heat", train_fraction=0.7):
    ...
    base_dataset = tf.data.TextLineDataset(URL)
    ...
    with tf.Session() as sess:
        ...
        print('train', sess.run(train.reduce(np.int64(0), lambda x, _: x + 1)))
        print('test', sess.run(test.reduce(np.int64(0), lambda x, _: x + 1)))
...
#     df = pd.read_csv(URL, names=types.keys(), dtype=types, na_values="?")
#
#     return df

#
# def load_data(y_name="should-heat", train_fraction=0.7, seed=None):
